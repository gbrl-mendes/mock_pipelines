      ...
      for (PRJ in all_projects) {
        ...
        project_data <- residual_hits %>%
          ...
          dplyr::mutate(
            ...
            PctReads = Count / `Total reads` * 100)

        if (nrow(project_data) == 0) next

        # 2. Calculate dynamic dimensions based on actual data
        nsamples <- length(unique(project_data$Unique_File_name_Read))
        nprimers <- length(unique(project_data$Primer))

        # 3. Create the plot object
        primers_tile_residual <-
          project_data %>%
          ggplot2::ggplot(ggplot2::aes(y = Unique_File_name_Read)) +

          # Bar layer
          ggplot2::geom_bar(ggplot2::aes(x = `Total reads`,
                                         fill = PctReads,
                                         color = Primer,
                                         group = Sequences),
                            stat = "identity",
          ...
          # Dynamic horizontal lines based on actual sample count
          ggplot2::geom_hline(yintercept = seq(0.5, nsamples + 0.5, 2),
                              color = "darkgrey") +
        ...
        ggplot2::ggsave(file = paste0(results_path, "/", PRJ,
                        ...
                        device = "pdf",
                        width = 10 + nprimers * 10,
                        height = nsamples * 0.3 + 3,
                        units = "cm",
                        limitsize = FALSE,
                        dpi = 600)
