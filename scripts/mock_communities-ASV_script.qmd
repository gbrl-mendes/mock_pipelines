...
-   **Run FastQC and MultiQC by project.** Creates per‑project folders and runs quality control in batch using system calls.
    ...
    for (PRJ in all_projects) {
      ...
      # 3. Show MultQC report
        browseURL(file.path(multiqc_path, 
                    paste0(PRJ, "--primer_removal_quality-MultiQC_report.html")))
    }
    ```
      ...
      if (length(fnFs_in) > 0) {
        for (i in seq_along(fnFs_in)) {
          system2(command = cutadapt, 
                  args = c(flags$R1_flags, flags$R2_flags,
                       "-e = 0.1",
                       "-j = 70",
                       "-n", 5,
                       "--minimum-length = 30",
...
After running Cutadapt, it is important to confirm that primer sequences were effectively removed from all reads before proceeding to quality filtering and ASV inference. All counts are expected to be zero or near-zero; any sample showing consistent primer hits after trimming should be investigated for unexpected primer orientations, chimeric primers, or incorrect flag configuration in the Cutadapt step.
...
-   **Count primer hits in Cutadapt output.** Apply the `**primerHits**` function across all trimmed samples, mirroring the approach used during raw read inspection.
    ...
    ```{r}
    # Collect all Cutadapt output files present on disk
    ...
    purrr::map_dfr(primers_all_orients %>%
                   ...
                   function(PRIMER_SET) {
                     ...
                     rbind(primerHits(primer = fwd_seq, fn = R1),
                           primerHits(primer = rev_seq, fn = R2)) %>%
...
-   **Execute filtering.** Run the filtering function in parallel across all samples and generate a summary table of read retention.
    ...
    list(filter_files_per_primer$Unique_File_name_Primer,
...
repair_files_per_primer <- cut_files_per_primer %>%
  ...
  tidyr::pivot_wider(id_cols = c("Unique_File_name", "Unique_File_name_Primer", "Primer", "Run"),
                     names_from = "Stage",
                     values_from = "Read file") %>% 
  dplyr::mutate(FWD_R1_repaired = stringr::str_replace_all(FWD_R1_cutadapt, "cutadapt", "repaired"),
...
all_filtered_out_fun_repaired2 <- 
  furrr::future_pmap_dfr(list(repair_files_per_primer$Unique_File_name_Primer,
...
all_repaired <- all_filtered_out_fun_repaired2 %>% 
  ...
  dplyr::mutate(Proportion = round((`Repaired reads` / 
                                      `Primer detected reads` * 100), 
                                   digits = 2)) %>% 
  BiocGenerics::unique()
