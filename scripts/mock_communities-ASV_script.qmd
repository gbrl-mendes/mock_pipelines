...
-   **Count primer hits in Cutadapt output.** Apply the `**primerHits**` function across all trimmed samples, mirroring the approach used during raw read inspection.

    ```{r}
    # Apply primerHits to all cutadapt output files
    primers_in_cutadapt_reads <- purrr::pmap_dfr(
      list(cutadapt_check_files$Unique_File_name_Primer,
       cutadapt_check_files$FWD_R1_cutadapt,
       cutadapt_check_files$FWD_R2_cutadapt),
      function(sample_name, R1, R2) {

    purrr::map_dfr(primers_all_orients %>%
                     dplyr::pull(`Primer pair`) %>%
                     unique(),
                   function(PRIMER_SET) {
                     
                     fwd_seq <- primers_all_orients$Sequence[grep(paste0(str_extract(PRIMER_SET, 
                                                                                     "^[^_]+"), "_FWD_Forward"),
                                                                  primers_all_orients$`Orientation name`)]
                     
                     rev_seq <- primers_all_orients$Sequence[grep(paste0(str_extract(PRIMER_SET, 
                                                                                     "^[^_]+"), "_REV_Forward"),
                                                                  primers_all_orients$`Orientation name`)]
                     
                     rbind(primerHits(primer = fwd_seq, file_name = R1),
                           primerHits(primer = rev_seq, file_name = R2)) %>%
...
-   **Prepare data.** Transform the primer hits table into long format for visualization, filtering out zero counts.
    ...
    # Inspect: if this table is empty, primer removal was successful across all samples
    primers_in_cutadapt_reads_long %>%
      dplyr::filter(Count != 0)

    ```

-   **Generate and save plots.** If residual primers are detected, visualize their distribution across samples using the same plotting approach as in the raw read inspection step.

    The plot uses the same structure as the raw primer visualization, allowing direct comparison between pre- and post-Cutadapt primer counts. If all primer removal was successful, the filtered table will be empty and no plot needs to be saved.

    ```{r}
    if (nrow(dplyr::filter(primers_in_cutadapt_reads_long, Count != 0)) > 0) {
      ...
      for (PRJ in all_projects) {
        ...
        # 1. Filter and prepare data ONCE for the current project
        # This avoids filtering the large dataset multiple times inside the loop
        project_data <- primers_in_cutadapt_reads_long %>%
          dplyr::filter(Count != 0,
                        Project == PRJ) %>%
          dplyr::mutate( 
        ...
        # 3. Create the plot object
        primers_tile_cutadapt <- project_data %>%
          ...
          # Titles and Facets
          ggplot2::labs(title = paste0(PRJ, " — Post-Cutadapt Primer Check"),
                        subtitle = "Residual primer hits after Cutadapt trimming.\nAll counts should be zero. Non-zero values require investigation.",
                        x = "Primers",
                        y = "Sample") +
          ggplot2::facet_grid(rows = vars(Primer_lbl),
                              cols = vars(Sequences_lbl),
        ...
        # 4. Save the plot
        ggplot2::ggsave(
          file = paste0(results_path, "/", PRJ, "-primers_found_after_cutadapt.pdf"),
          plot = primers_tile_cutadapt,
          device= "pdf",
          width = 10 + (n_primers * 10),
          height = (n_samples * 0.3) + 3,
          units = "cm",
